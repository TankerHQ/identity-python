...
def test_hash():
    ...
    buffer = b'abc'
    output = tankersdk.crypto.generichash(buffer, size=64)
...
def test_valid_signature_hard_coded(test_trustchain):
    ...
    signature = tankersdk.crypto.sign_detached(message, private_key)
    tankersdk.crypto.verify_sign_detached(message, signature, public_key)


def test_valid_signature_generated_keys():
    message = b'message'
    public_key, private_key = tankersdk.crypto.sign_keypair()
    signature = tankersdk.crypto.sign_detached(message, private_key)
    tankersdk.crypto.verify_sign_detached(message, signature, public_key)
...
def test_sign_invalid_message():
    message = b'message'
    public_key, secret_key = tankersdk.crypto.sign_keypair()
    signature = tankersdk.crypto.sign_detached(message, secret_key)

    invalid_message = b'm3ss4ge'
    with pytest.raises(tankersdk.crypto.InvalidSignature):
        tankersdk.crypto.verify_sign_detached(invalid_message, signature, public_key)
...
def test_sign_invalid_signature():
    ...
    with pytest.raises(tankersdk.crypto.InvalidSignature):
        tankersdk.crypto.verify_sign_detached(message, invalid_signature, public_key)
